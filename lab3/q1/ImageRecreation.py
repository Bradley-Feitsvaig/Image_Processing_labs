...
def save_image_and_calculate_mse(recreated, output_image):
    ...
    cv2.imwrite(f'recreated_{output_image}.jpg', recreated)
    mse(f'recreated_{output_image}.jpg', f'{output_image}.jpg')
...
if __name__ == '__main__':
    ...
    # image 4
    # averaging kernel oriented on Y-axis.
    kernel_size = 15
    ...
    # image 5
    # Part of sharpening image method.
    ...
    # image 6
    # Laplacian kernel for horizontal edges
    kernel = np.array([[1], [0], [-1]])
    recreated = signal.convolve2d(image, kernel, mode='same', boundary='wrap')
    save_image_and_calculate_mse(recreated, 'image_6')

    # # image 7
    # convolution that moves the whole image down/up cyclically (image height) /2 pixels.
    H, W = image.shape
